...
SinglyLinkedListNode* p= new SinglyLinkedListNode(x);
 if(head == NULL || head->data >= p->data){
    ...
    SinglyLinkedListNode* g=new SinglyLinkedListNode (0);
    for(SinglyLinkedListNode*k=head;k!=NULL;k=k->next){
        if(p->data<k->data){
       SinglyLinkedListNode* h=new SinglyLinkedListNode(p->data);
       h->next=k;
    g->next=h;
    break;
        }
